...
1.  **`resolve_package_metadata(package_name)`**: This is the central function of the script. It orchestrates the entire data-gathering process.
    ...
    -   **Definitive Path Resolution Logic:** The most critical design choice is the multi-step process for resolving the package's installation path, which avoids unreliable methods.
        1.  **`top_level.txt` Analysis:** It first reads the `top_level.txt` file from the package's `.dist-info` directory. This file provides the actual importable module name(s), which can differ from the `pip` package name (e.g., `Pillow` vs. `PIL`).
        ...
        4.  **`importlib.util.find_spec` Fallback:** If the constructed path does not exist, it falls back to `importlib.util.find_spec()`. This function leverages Python's import system directly to find how and where a module would be loaded, making it the ultimate source of truth for the module's location. This is especially important for complex installations, namespace packages, and editable installs.
